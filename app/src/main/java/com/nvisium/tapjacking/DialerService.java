	...
	private void launchDialer() {
		...
		Thread t = new Thread() {
			public void run() {
				...
				//intent.setData(Uri.parse("eu.netinfo.colpatria.system"));
				//getApplication().startActivity(intent);

				//Intent intent = new Intent();
				//intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				//intent.setComponent(new ComponentName("eu.netinfo.colpatria.system", "eu.netinfo.colpatria.system.activity_login"));
				//startActivity(intent);

				Intent intent = getPackageManager().getLaunchIntentForPackage("eu.netinfo.colpatria.system");
				if (intent != null) {
					...
					intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					intent.setData(Uri.parse("market://details?id=" + "eu.netinfo.colpatria.system"));
					startActivity(intent);
				}
